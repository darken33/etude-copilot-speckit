...
public interface ConnaissanceClientApi {
    ...
    @Operation(
        ...
        description = "Modifie uniquement l'adresse d'une fiche de connaissance client existante.  **Cas d'usage :** - Déménagement du client - Correction d'une adresse erronée - Mise à jour suite à un changement de domicile  **Validation :** - Le client doit exister (erreur 404 sinon) - L'adresse est validée via l'API IGN (code postal/ville cohérents) - En cas d'échec de validation externe, erreur 422  **Comportement :** - Seule l'adresse est modifiée (nom, prénom, situation inchangés) - Un événement Kafka est publié pour notifier le changement d'adresse - Audit trail avec traçabilité complète - Validation externe avec circuit breaker (résilience)  **Performance :** - Temps de réponse typique : < 2s (avec validation API IGN) - Circuit breaker actif pour l'API IGN (3 échecs → skip 60s) ",
    ...
    default ResponseEntity<ConnaissanceClientDto> changerAdresse(
        @Parameter(name = "id", description = "Identifiant unique de la fiche client au format UUID. La fiche doit exister et être accessible à l'utilisateur. ", required = true, in = ParameterIn.PATH) @PathVariable("id") UUID id,
    ...
    @Operation(
        ...
        description = "Modifie uniquement la situation familiale et le nombre d'enfants d'un client existant.  **Cas d'usage :** - Changement d'état civil (mariage, divorce, veuvage) - Naissance ou adoption d'enfant(s) - Mise à jour suite à un événement de vie - Correction d'informations familiales erronées  **Validation :** - Le client doit exister (erreur 404 sinon) - La situation familiale doit être valide (CELIBATAIRE, MARIE, DIVORCE, VEUF, PACSE) - Le nombre d'enfants doit être entre 0 et 20 - Cohérence métier : un célibataire peut avoir des enfants  **Comportement :** - Seule la situation familiale est modifiée (nom, prénom, adresse inchangés) - Aucun événement Kafka n'est publié (pas de changement d'adresse) - Audit trail avec traçabilité complète - Opération transactionnelle  **Règles métier :** - Un client célibataire peut avoir des enfants (garde alternée, adoption, etc.) - Le nombre d'enfants peut être 0 même si marié - La situation PACSE est acceptée (Pacte Civil de Solidarité)  **Performance :** - Temps de réponse typique : < 100ms - Pas de validation externe requise ",
        tags = { "ConnaissanceClient" },
        responses = {
    ...
    default ResponseEntity<Void> deleteConnaissanceClient(
        @Parameter(name = "id", description = "Identifiant unique de la fiche à supprimer au format UUID. La fiche doit exister et être accessible à l'utilisateur. ", required = true, in = ParameterIn.PATH) @PathVariable("id") UUID id
    ) {
    ...
    default ResponseEntity<ConnaissanceClientDto> getConnaissanceClient(
        @Parameter(name = "id", description = "Identifiant unique de la fiche de connaissance client au format UUID. Cet identifiant est généré automatiquement lors de la création. ", required = true, in = ParameterIn.PATH) @PathVariable("id") UUID id
    ) {
    ...
    @Operation(
        ...
        description = "Modifie de manière atomique et complète une fiche de connaissance client existante.  **⚠️ ATTENTION : Modification globale (PUT)** Cette opération remplace l'intégralité de la fiche client. Tous les champs obligatoires doivent être fournis.  **Cas d'usage :** - Mise à jour complète des informations client - Correction de données erronées - Synchronisation avec un système externe - Migration de données  **Validation :** - Tous les champs obligatoires doivent être fournis - Validation du format des données (code postal, noms, etc.) - Vérification externe de l'adresse via API IGN - Cohérence de la situation familiale  **Comportement :** - Opération transactionnelle (tout ou rien) - Publication d'un événement Kafka si l'adresse change - Audit trail automatique avec traçabilité complète - Validation métier multi-niveaux (DTO, domain, externe)  **Sécurité :** - Authentification JWT requise - Rate limiting : 5 requêtes/seconde par utilisateur - Rôle ADMIN ou CONSEILLER requis - Audit trail obligatoire  **Performance :** - Temps de réponse typique : < 100ms (sans changement d'adresse) - Temps de réponse typique : < 2s (avec validation externe d'adresse) ",
        tags = { "ConnaissanceClient" },
        responses = {
            @ApiResponse(responseCode = "200", description = "Fiche client modifiée avec succès. Retourne la fiche complète avec toutes les informations à jour. ", content = {
                @Content(mediaType = "application/json", schema = @Schema(implementation = ConnaissanceClientDto.class))
            }),
            @ApiResponse(responseCode = "400", description = "Requête invalide - Données manquantes ou format incorrect. Vérifier que tous les champs obligatoires sont fournis. ", content = {
                @Content(mediaType = "application/json", schema = @Schema(implementation = ApiErrorResponseDto.class))
            }),
            @ApiResponse(responseCode = "404", description = "Client non trouvé - L'identifiant fourni ne correspond à aucune fiche. ", content = {
                @Content(mediaType = "application/json", schema = @Schema(implementation = ApiErrorResponseDto.class))
            }),
            @ApiResponse(responseCode = "422", description = "Entité non traitable - Adresse invalide (code postal/ville incompatibles). Vérification externe via API IGN a échoué. ", content = {
                @Content(mediaType = "application/json", schema = @Schema(implementation = ApiErrorResponseDto.class))
            }),
    ...
    default ResponseEntity<ConnaissanceClientDto> modifierClient(
        @Parameter(name = "id", description = "Identifiant unique de la fiche à modifier au format UUID. La fiche doit exister et être accessible à l'utilisateur. ", required = true, in = ParameterIn.PATH) @PathVariable("id") UUID id,
        @Parameter(name = "ConnaissanceClientInDto", description = "La fiche de connaissance client complète à enregistrer. Tous les champs obligatoires doivent être fournis même s'ils ne changent pas. ", required = true) @Valid @RequestBody ConnaissanceClientInDto connaissanceClientInDto
    ) {
        return getDelegate().modifierClient(id, connaissanceClientInDto);
    }
